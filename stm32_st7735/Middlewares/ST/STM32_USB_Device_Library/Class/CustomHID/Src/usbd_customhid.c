...
static const display_info_t display_info = {
    ...
    case USB_REQ_TYPE_CLASS :
      switch (req->bRequest)
      {
          ...
          if ((req->wValue >> 8) == 0x3) {
              switch(req->wValue & 0xFF) {
                  ...
                  case GLCD2USB_RID_WRITE_128:
                      hhid->ReportId = GLCD2USB_RID_WRITE;
                      //cmd_state.offset = 0xffff;
                      break;

                  case GLCD2USB_RID_SET_ALLOC:
                      print("-> set alloc\n");
                      hhid->ReportId = GLCD2USB_RID_SET_ALLOC;
                      break;

                  case GLCD2USB_RID_SET_BL:
                      print("-> set backlight\n");
                      hhid->ReportId = GLCD2USB_RID_SET_BL;
                      break;
              }
        ...
        case CUSTOM_HID_REQ_GET_REPORT:
          //print("setup USB_REQ_TYPE_CLASS CUSTOM_HID_REQ_GET_REPORT wValue:%d\n", req->wValue);
          if ((req->wValue >> 8) == 0x3) {
            if ((req->wValue & 0xFF) == GLCD2USB_RID_GET_INFO) {
                print("<- get display info\n");
                memcpy(hhid->Report_buf, &display_info, sizeof(display_info_t));
                USBD_CtlSendData(pdev, (uint8_t *)(void *)&hhid->Report_buf, sizeof(display_info_t));
                break;
            } else if ((req->wValue & 0xFF) == GLCD2USB_RID_GET_BUTTONS) {
                //print("<- get buttons\n"); 
                hhid->Report_buf[0] = GLCD2USB_RID_GET_BUTTONS; 
                hhid->Report_buf[1] = 0x00;
                //hhid->Report_buf[1] = button_map_get();  //TODO: get key value
                USBD_CtlSendData(pdev, (uint8_t *)(void *)&hhid->Report_buf, 2);
                break; 
            }
          }
          break;
        ...
        default:
          print("setup USB_REQ_TYPE_CLASS req->bRequest:%d\n", req->bRequest);
